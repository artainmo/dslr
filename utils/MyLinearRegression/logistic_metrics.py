...
def binary_set(predicted_values):
    ...
    while i < predicted_values.shape[0]:
        if predicted_values[i] < 0.5: #To trade-off precision and recall 0.5 can become 0.7 for example if you want to avoid false positives for cancer, if you want to avoid false negatives you can trade off recall for precision by lowering to 0.3 for example
